...
class VAETrainer(BaseTrainer):
    ...
    def inner_step(
        ...
        with torch.autocast('xla', dtype=torch.bfloat16, enabled=self.config.trainer.use_autocast):
            reconstruction_loss, kl_sum, lm_states, lm_grad = (
                self.microbatch_forward(
                    input_ids,
                    assistant_mask,
                    attention_mask,
                    alpha,
        ...
        return reconstruction_loss, kl_sum


    def microbatch_forward(
        self,
        input_ids,
        assistant_mask,
        attention_mask,
        ...
        input_ids, assistant_mask, valid_mask = tuple(
            maybe_shard_with_gradients(tensor)
            for tensor in (input_ids, assistant_mask, attention_mask)
        )
        target_mask = assistant_mask[:, 1:] & valid_mask[:, 1:]

        lm_states, _, radius = self.model(
            input_ids=input_ids,
    ...
    @torch_xla.compile(full_graph=True)
    def post_forward(self, reconstruction_loss):
    ...
    def train_step(self, batch: dict) -> tuple[torch.Tensor, dict, torch.Tensor]:
        input_ids, assistant_mask, attention_mask = self.flatten_episodes(
            batch["input_ids"],
        ...
        if sequence_count % accumulation_steps != 0:
            raise ValueError(
                f"flattened sequence count {sequence_count} must be divisible "
                f"by gradient_accumulation_steps={accumulation_steps}"
            )

        target_mask = assistant_mask[:, 1:] & attention_mask[:, 1:]
        assistant_count = target_mask.long().sum()
        atom_count = attention_mask.long().sum()
        denominator = assistant_count.clamp_min(1).float()
        alpha = self._initialize_alpha(assistant_count, sequence_count)
        noise_scale = cosine_warmup(
            self.model.noise_step.float(),
            self.config.trainer.noise_warmup_steps,
        )
        # Materialize replicated schedule state before the first compiled
        # microstep captures and reads it.
        torch_xla.sync(wait=True)

        reconstruction_losses = []
        kl_sums = []
        for microbatch in zip(
            input_ids.chunk(accumulation_steps),
            assistant_mask.chunk(accumulation_steps),
            attention_mask.chunk(accumulation_steps),
        ):
            reconstruction_loss, kl_sum = self.inner_step(
                *microbatch,
                alpha,
                denominator,
                noise_scale,
            )
            # Compiled calls are asynchronous; the next microstep must not read
            # a gradient buffer before this microstep has updated it.
            torch_xla.sync(wait=True)
            reconstruction_losses.append(reconstruction_loss)
            kl_sums.append(kl_sum)

        reconstruction_loss = torch.stack(reconstruction_losses).sum()
        total_kl = torch.stack(kl_sums).sum()
        kl_per_assistant_token = total_kl / denominator
        loss = reconstruction_loss + kl_per_assistant_token

        post_aux, grad_norm = self.post_forward(reconstruction_loss)
        torch_xla.sync(wait=True)

        aux = {
            "reconstruction_loss": reconstruction_loss,

            "kl": total_kl/sequence_count,
