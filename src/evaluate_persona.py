...
DEFAULT_NUM_EXAMPLES = [0] + list(2 ** i for i in range(0, 9))  # 0, 1, 2, 4, 8, ..., 256
...
def save_results(args, label: int | str, results):
    if args.fresh_config is None:
        path = Path(constants.LOCAL_DATA_PATH) / "persona_results" / ((args.save_name+"/"+args.checkpoint.replace("/", "--")) if args.save_name is not None else args.checkpoint.replace("/", "--")) / f"{label:012d}.json"
    else:
        path = Path(constants.LOCAL_DATA_PATH) / "persona_results" / (args.save_name if args.save_name is not None else "fresh") / Path(args.fresh_config).stem / f"{label}.json"
