...
DEFAULT_DATASET = "aklein4/ManyICLBench-SmolLM2-1024-natural-instructions-format"

DEFAULT_NUM_EXAMPLES = [0] + list(2 ** i for i in range(9)) + [384]
...
def exact_match(input_ids, assistant_mask, logits, special_token_ids):
    labels = input_ids[:, 1:]
    mask = assistant_mask[:, 1:]
    # Chat templates include control tokens such as <|eot_id|> in the
    # assistant mask.  Benchmark exact match concerns the textual answer, not
    # whether the model predicts the template's conversation terminator.
    for token_id in special_token_ids:
        mask = mask & labels.ne(token_id)
...
def evaluate(model, logits_fn, tokenizer, rows, args, device):
    ...
    for test_idx in tqdm(range(args.num_eval), desc="evaluating", leave=False):
        ...
        if args.eval_fn == "exact_match":
            score = exact_match(input_ids, assistant_mask, logits, tokenizer.all_special_ids)
...
def save_results(args, label: int | str, results):
    if args.fresh_config is None:
        path = Path(constants.LOCAL_DATA_PATH) / "icl_acc_results" / ((args.save_name+"/"+args.checkpoint.replace("/", "--")) if args.save_name is not None else args.checkpoint.replace("/", "--")) / f"{label:012d}.json"
    else:
        path = Path(constants.LOCAL_DATA_PATH) / "icl_acc_results" / (args.save_name if args.save_name is not None else "fresh") / Path(args.fresh_config).stem / f"{label}.json"
